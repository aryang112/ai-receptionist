...
function call() {
  ...
  c.session = { appendTwilioAudio: () => {}, truncateActiveResponse: () => {}, close: () => {} };
  c.streamSid = 'S_scn';
  c.callSid = 'CA_scn';
  return c;
}
...
async function avail(label: string, serviceName: string, date: string, preferredTime?: string) {
  const r = await call().handleSuggestAvailability({ serviceName, date, ...(preferredTime ? { preferredTime } : {}), searchNearby: true });
  const verdict = r.notOffered
    ? `notOffered → closest: ${(r.closest ?? []).join(', ')}`
    : r.ambiguous
      ? `ambiguous → ${JSON.stringify(r.ambiguous).slice(0,140)}`
      : `service="${r.service}"  times=${(r.slots ?? []).map((s: any) => s.time).slice(0, 4).join(', ') || 'none'}${r.alternativeDates ? `  nearby=${r.alternativeDates.length}d` : ''}`;
...
(async () => {
  ...
  await avail('05 — threading+lip phrase read as a STAFF NAME (Sept 10, still open in register)', 'eyebrow threading and upper lip', TOMORROW);
  await avail('01 — a person is not a service', 'Richa', TOMORROW);
  await avail('03 — caller wording vs catalog name', 'eyebrow threading', TOMORROW);
  await avail('O06 — "henna brows" must NOT silently become Brow Threading', 'henna brows', TOMORROW);
  await avail('07 — full treatment vs touch-up', 'microblading touch-up', TOMORROW);
  await avail('06 — eyebrow tattoo → full micro blading/shading', 'eyebrow tattoo', TOMORROW);
  await avail('unknown service — must never say "not in our system"', 'microneedling', TOMORROW);
  await avail('ambiguous family', 'wax', TOMORROW);

  console.log('\n════ TIME & DATE HANDLING ════');
  await avail('11 — a broad preference must survive', 'brow threading', TOMORROW, '16:00');
  await avail('R13 — right at closing time', 'lash lift', TOMORROW, '18:45');
  await avail('closed day (Sunday) — must say CLOSED, never "fully booked"', 'brow threading', SUNDAY);
