Archived `state.md` entries — AI Receptionist (Erica)
Verbatim entries dated 2026-07-19 through 2026-09-11 (plus the undated "Current status"/"Architecture"/"What's DONE"/"PENDING"/"How to run a live test"/"Diagnostics"/"Key files" block that used to sit mid-file), moved out of `state.md` verbatim and in original order by the W1 context-diet pass (2026-09-16). Nothing here was edited. See `state.md`'s CURRENT section for present status.
